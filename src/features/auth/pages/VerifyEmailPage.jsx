  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setStatus('error');
        showError(err.response?.data?.message || 'El enlace es inválido o expiró');
      }
    })();
    ...
    <main className="relative min-h-screen w-full font-sans text-on-base flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md bg-surface-2 border-[3px] border-stroke-strong rounded-3xl p-8 text-center shadow-brutal">
        <div className="flex justify-center mb-4">
          ...
          {status === 'success' && <CheckCircle2 className="text-secondary" size={56} />}
        ...
        <p className="text-sm text-on-base-muted mt-2">
          ...
          {status === 'success' && 'Tu cuenta ya está activa. Inicia sesión y a comer.'}
          {status === 'error' && 'El enlace puede estar incompleto o haber expirado.'}
        </p>
