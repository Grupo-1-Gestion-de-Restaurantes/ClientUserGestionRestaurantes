...
export const DashboardLayout = () => {
  ...
  return (
    <main className="relative h-screen font-sans text-on-base overflow-hidden flex flex-col lg:flex-row">
      ...
      {/* Desktop Sidebar */}
      <div className="hidden lg:block shrink-0 h-full border-r-[3px] border-stroke-soft bg-surface-1/50 backdrop-blur-sm relative z-50">
        <DashboardSidebar />
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row h-full overflow-hidden">
        {/* Page Content */}
        <div className="flex-1 p-4 sm:p-6 lg:p-10 pt-16 lg:pt-10 overflow-y-auto custom-scrollbar">
          <Outlet />
        </div>

        {/* Desktop Cart Panel */}
        {!isCartCollapsed && (
          <div className="hidden lg:block w-[420px] p-6 lg:p-10 shrink-0 h-full border-l-[3px] border-stroke-soft bg-surface-1/50 backdrop-blur-sm">
      ...
      {navOpen ? (
        <div className="lg:hidden fixed inset-0 z-[70]">
          ...
          <motion.div
            ...
            className="absolute left-0 top-0 bottom-0 w-[300px] p-4 flex flex-col bg-surface-1"
          >
            <DashboardSidebar
              variant="drawer"
            ...
            <button
              type="button"
              onClick={() => setNavOpen(false)}
              className="mt-3 w-full shrink-0 rounded-2xl bg-surface-2 border-[3px] border-stroke-strong py-3 text-sm font-bangers tracking-widest text-on-base flex items-center justify-center gap-2 hover:text-primary transition-colors"
            >
              <X size={18} />
              CERRAR
            </button>
